...
-  First parameter: When the user provides ONLY a PR number (e.g., "1694" or "#1694"), automatically execute the full workflow. If the user provides "local", then jump to step 3 and proceed from there, mocking steps if test mode only (see next parameter)
-  Second parameter (optional): When the user provides also "test" or "dry-run" or similar, all UPDATE steps are mocked and set as complete and the only ALLOWED API CALLS are for account login and switch. Login and Switch is always done to ensure at least auth testing.
...
Store the current gcloud account, as there will be a switch to a service account, and when done it should switch back to the current user, can be stored as environment variable and then cleared at the end.

```sh
# Get current gcloud account
ORIGINAL_ACCOUNT=$(gcloud config list account --format "value(core.account)")
```

Use the following `curl` command and the service account key file in `.json` form, at the root of the project: `gtm-service-account-key.json`, which will result in an access token, to be set as environment variable `ACCESS_TOKEN` (to be used in update requests).
...
# Get access token
ACCESS_TOKEN=$(gcloud auth print-access-token --scopes=https://www.googleapis.com/auth/tagmanager.readonly,https://www.googleapis.com/auth/tagmanager.edit.containers)
```
...
-  A parameter LATEST_WORKSPACE needs inferring, so we also need to fetch the latest workspace ID via the API.

```sh
# Get all workspaces and find the latest one
LATEST_WORKSPACE=$(curl -H "Authorization: Bearer $ACCESS_TOKEN" \
...
"https://tagmanager.googleapis.com/tagmanager/v2/accounts/4701935119/containers/11882799/workspaces/${LATEST_WORKSPACE}/variables" \
-d '{
...
curl -X POST \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  "https://tagmanager.googleapis.com/tagmanager/v2/accounts/4701935119/containers/11882799/workspaces/${LATEST_WORKSPACE}/triggers" \
  -d '{
    "name": "'"${INFERRED_VAR_NAME}"' Trigger",
    "type": "CUSTOM_EVENT",
    "customEventFilter": [
      {
        "type": "EQUALS",
...
```sh
CREATED_TRIGGER_ID=$(curl -H "Authorization: Bearer $ACCESS_TOKEN" \
  "https://tagmanager.googleapis.com/tagmanager/v2/accounts/4701935119/containers/11882799/workspaces/${LATEST_WORKSPACE}/triggers" \
  | jq -r '.trigger[] | select(.name == "'"${INFERRED_VAR_NAME}"' Trigger") | .triggerId')
```

#### 5.3 ADD TRIGGER TO G4A TAG

-  Run the following command to add the trigger to the specific G4A tag (167), using the trigger ID value CREATED_TRIGGER_ID stored from the previous step.

```sh
curl -H "Authorization: Bearer $ACCESS_TOKEN" \
  "https://tagmanager.googleapis.com/tagmanager/v2/accounts/4701935119/containers/11882799/workspaces/${LATEST_WORKSPACE}/tags/167" \
...
#### 5.3.1 CHECK ADDED TRIGGER TO G4A TAG
...
### 7. SWITCH TO ORIGINAL gcloud ACCOUNT

```sh
gcloud config set account $ORIGINAL_ACCOUNT
unset ACCESS_TOKEN
unset LATEST_WORKSPACE
unset ORIGINAL_ACCOUNT
```
