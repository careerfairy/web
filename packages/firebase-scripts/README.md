...
3. Run the [createNewFieldsAndLevelOfStudyInFirestore script](src/migrations/fieldsOfStudy/createNewFieldsAndLevelOfStudyInFirestore/README.md) that will make the new field and level of study root collections
4. Double check in the emulators/Prod DB if the new collections exist
5. Run the [createCustomGroupCategorySubCollections script](src/migrations/groups/createCustomGroupCategorySubCollections/README.md) that will convert group categories into a collection of questions found in `careerCenterData/{groupId}/groupQuestions`
...
7. Run the [storeGroupQuestionsOnLivestreams script](src/migrations/livestreams/storeGroupQuestionsOnLivestreams/README.md) that backfills all livestreams with the questions and answers of the groups in the groupIds array field. It is possible that on the emulators the script will stall if you have the UI or webapp running and listening to the livestreams collection
8. Double check in the emulators/Prod DB that the questions for the event are stored on the livestream
9. Run the [backfillUserData script](src/migrations/users/backfillUsers/README.md) that backfills the users with a field and level of study based on their event registrations and the mapping JSON file.
10.   Double check if your user has been assigned a field/level of study or null.
11.   Run this [backfill registered/participating/talentpool users script](src/migrations/users/backfillUsers/README.md) first for talentPool, then participatingStudents and lastly registeredStudents.
12.   Double check that the new `livestreams/{livestreamId}/userLivestreamData` collections exist.
13.   If using emulators Migration is complete
14.   Merge PR into Develop then release
