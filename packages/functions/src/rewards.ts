 ...
 */
export const rewardLivestreamInvitationComplete = onCall(async (request) => {
   const userEmail = request.auth?.token?.email
   const livestreamId = request.data.livestreamId
   ...
   functions.logger.info("Created a new reward for the livestream attendance")
})

/**
 * Generic function to create a reward for a user progression action
 */
export const rewardUserAction = onCall(async (request) => {
   const userEmail = request.auth?.token?.email
   const action = request.data.action

   functions.logger.debug(userEmail, request.data)
