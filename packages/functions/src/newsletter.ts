...
export const manualTemplatedEmail = onRequest(
   runtimeSettings,
   async (req, res) => {
      logger.info("manualTemplatedEmail: v11.0 - X-mas testimonial")
      ...
      if (receivedEmails.length === 1 && receivedEmails[0] === "everyone") {
         ...
         res.status(200).send("X-mas testimonial email sent to everyone")
      } else {
         await sendManualTemplatedEmail(receivedEmails)
         res.status(200).send(
            "X-mas testimonial email sent to " + receivedEmails.join(", ")
         )
      }
   }
)
...
async function sendManualTemplatedEmail(overrideUsers?: string[]) {
   if (newsletterAlreadySent) {
      logger.info("X-mas testimonial email already sent, skipping")
      return
   }
   ...
   logger.info("X-mas testimonial execution done")
}
