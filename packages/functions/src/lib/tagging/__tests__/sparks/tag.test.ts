 ...
 * Sparks, where the consistency of the data must be ensured taking into consideration other customJobs which might be linked
 * to the Sparks as well as prevent tag duplication and other modification scenarios.
 *
 *
 * With the way function syncCustomJobLinkedContentTags was implemented in a generic way, it leaves the burden
 * of determining customJob links (to Sparks in this case) via async functions passed as parameters, and during these unit tests
 ...
 * since it will not have a view of all existing jobs. Locally created customJobs or Sparks can be added to this list, see example in
 * test 'Should add spark tags if link is added to customJob'.
 */
