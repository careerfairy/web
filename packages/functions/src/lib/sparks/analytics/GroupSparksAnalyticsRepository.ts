   ...
   implements IGroupSparksAnalyticsRepository
{
   ...
   async getTopCompaniesByIndustry(
      ...
      const topCompaniesBuilders = new TopCompaniesBuilder(
         bigQueryResults,
         INDUSTRY_SPARKS_LIMIT,
         this.groupId
      )

      const topCompaniesBuildersInitialized = await topCompaniesBuilders
         .buildTopCompaniesByIndustrySparksIdsLookup()
         .fetchSparksData(this.sparksRepo)

      const topCompaniesData = await topCompaniesBuildersInitialized
