...
export interface IRecommendationService {
   ...
   web: {
      user: (IUserCreation<WebUserInfoType> | IUserUpsertion<WebUserInfoType>) &
         IUserDeletion<WebUserInfoType>
   }
   console: {
      user:
         | IUserCreation<ConsoleUserInfoType>
         | IUserUpsertion<ConsoleUserInfoType>
   }
}
...
export default class RecommendationServiceCore {
   ...
   constructor(serviceName: string, service: any, log: typeof logger) {
      ...
      try {
         ...
         this.log.warn(
            "Couldn't instantiate sync service:",
            (e as Error).message
         )
      }
   }
   ...
   async getUserLastBooking(user: {
      email: string
   }): Promise<{ booking: { createdAt: Date } | null } | null> {
...
export interface IRecommendationServices {
   new (): IRecommendationService
}
