...
export const convertStreamJsDatesToTimestamps = (stream, jsDateToTimestampCallback) => {
    const newStream = {...stream}
    if (newStream.createdDate) {
        newStream.created = jsDateToTimestampCallback(newStream.createdDate)
    }
    if (newStream.lastUpdatedDate) {
        newStream.lastUpdated = jsDateToTimestampCallback(newStream.lastUpdatedDate)
    }
