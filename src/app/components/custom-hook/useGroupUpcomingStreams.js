...
const pastLivestreamsQuery = [["start", "<", currentTime], ["start", ">", earliestTime]]

const useListenToGroupStreams = (livestreamId, groupId, selectedOptions, typeOfStreams = UPCOMING_LIVESTREAMS_NAME) => {

    const query = useMemo(() => {
        const isUpcoming = typeOfStreams === UPCOMING_LIVESTREAMS_NAME
        return [{
            collection: "livestreams",
            where: [["groupIds", "array-contains", groupId], isUpcoming ? [...upcomingLivestreamsQuery] : [...pastLivestreamsQuery], ["test", "==", false]],
    ...
    return useSelector(state =>
        groupUpcomingLivestreamsSelector(state.firestore.ordered[`${typeOfStreams} of ${groupId}`], {
            livestreamId,
            groupId,
            selectedOptions
        })
