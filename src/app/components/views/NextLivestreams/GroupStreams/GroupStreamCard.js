...
const useStyles = makeStyles((theme) => ({
    root: {
        ...
        paddingBottom: 15,
        textAlign: "left",
        WebkitBoxShadow: ({isHighlighted}) =>
            isHighlighted
                ? "0px -1px 11px 1px rgba(0,210,170,0.75)"
                : "0 0 5px rgb(180,180,180)",
        boxShadow: ({isHighlighted}) =>
            isHighlighted
                ? "0px -1px 11px 1px rgba(0,210,170,0.75)"
                : "0 0 5px rgb(180,180,180)",
        MozBoxShadow: ({isHighlighted}) =>
            isHighlighted
                ? "0px -1px 11px 1px rgba(0,210,170,0.75)"
                : "0 0 5px rgb(180,180,180)",
    },
...
const GroupStreamCard = ({livestream, user, fields, userData, firebase, livestreamId, id, careerCenterId, groupData, listenToUpcoming,}) => {
    ...
    const linkToStream = listenToUpcoming
        ? `/next-livestreams?livestreamId=${livestream.id}`
        : `/next-livestreams?careerCenterId=${groupData.groupId}&livestreamId=${livestream.id}`;

    const linkToUpcoming = listenToUpcoming
        ? `/upcoming-livestream?livestreamId=${livestream.id}`
        : `/upcoming-livestream?groupId=${groupData.groupId}&livestreamId=${livestream.id}`;
    ...
    useEffect(() => {
        if (livestream) {
            firebase
                .getDetailLivestreamCareerCenters(livestream.universities)
                .then((querySnapshot) => {
                    let groupList = [];
    ...
    const checkIfHighlighted = () => {
        if (
            ...
            return (
                livestreamId && !careerCenterId && !groupData.id && livestreamId === id
            );
    };
    ...
    function sendEmailRegistrationConfirmation() {
        return axios({
            method: "post",
            url:
                "https://us-central1-careerfairy-e1fd9.cloudfunctions.net/sendLivestreamRegistrationConfirmationEmail",
            data: {
                ...
                livestream_link:
                    "https://careerfairy.io/upcoming-livestream/" + livestream.id,
            },
        });
    }
        ...
        >
            <Fragment>
                ...
                >
                    <div className="date-indicator">
                        ...
                        <div>
                            <div style={{display: "inline-block"}}>
                                ...
                                />
                                {DateUtil.getPrettyDay(livestream.start.toDate())}
                            </div>
                    ...
                    >
                        <div
                            className="livestream-thumbnail-overlay"
                            style={{
                            ...
                            <div className="livestream-thumbnail-overlay-content">
                                ...
                                <div>
                                    <Button
                                        ...
                                        icon={
                                            user &&
                                            livestream.registeredUsers?.indexOf(user.email) > -1
                                                ? "delete"
                                                : "add"
                                        }
                                        color={
                        ...
                        <div
                            className={
                                "booked-icon animated tada delay-1s " +
                                (userIsRegistered() ? "" : "hidden")
                            }
                        >
                                ...
                                <Grid.Column width={14}>
                                    <div className="livestream-streamer-description">
                                        ...
                                        <div className="livestream-streamer">
                                            ...
                                            <div className="livestream-streamer-position light">
