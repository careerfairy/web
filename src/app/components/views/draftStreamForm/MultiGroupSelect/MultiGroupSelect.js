...
const MultiGroupSelect = ({
   ...
   const handleMultiSelect = (event, selectedOptions) => {
      ...
      let groupsWithOptions = selectedOptions.map((group) => {
         // if any of the groups from the multi select exists already in the selectedOptions
         handleAddTargetCategories(
            groupIdsArray,
            setTargetCategories,
            targetCategories
         );
         const oldGroup = selectedGroups.find(
            (selectedGroup) => selectedGroup.groupId === group.groupId
         );
         if (!oldGroup) {
            // state, please dont replace it!
      ...
      if (groupId) {
         const fixedGroupWithOptions = groups.find(
         ...
         if (fixedGroupId) {
            groupIdsArray = [
               fixedGroupId,
               ...groupIdsArray.filter(
                  (groupIdString) => groupIdString !== groupId
               ),
            ];
            groupsWithOptions = [
               fixedGroupWithOptions,
               ...groupsWithOptions.filter(
                  (group) => group.groupId !== groupId
               ),
            ];
         }
