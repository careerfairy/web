...
const UserDataModal = ({ isModalOpen, handleModalClose, missingFields }) => {
  ...
  return (
    ...
    >
      <DialogTitle
        // @ts-ignore
        component="div" sx={{ m: 0, p: 2 }} id="filldata-dialog-title">
      ...
      <DialogContent dividers>
        <Box mt={1}>
          <MultiStepWrapper steps={missingFields}
      ...
      <DialogActions sx={{ p: 2, justifyContent: "space-between" }}>
        <Box>
          <Typography variant="body2"
                      color="textSecondary">Step {currentFieldIndex + 1} of {missingFields.length}</Typography>
        </Box>
        <Box sx={{ textAlign: "right" }}>
          ...
          <Button onClick={handleNext} disabled={!isCurrentFieldComplete}
                  variant="contained">
            {isCurrentFieldComplete ? isLastField ? "Save" : "Next" : "Next"}
          </Button>
        </Box>
      </DialogActions>
    </Dialog>
