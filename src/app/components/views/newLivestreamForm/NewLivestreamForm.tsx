...
import {
  LanguageSelect
} from "../../helperFunctions/streamFormFunctions/components";
...
const NewLivestreamForm = () => {
    ...
    if ((livestreamId || draftStreamId) && allFetched) {
      (async () => {
        ...
        if (livestreamQuery.exists) {
          ...
          setSelectedGroups(existingGroups.filter(g => newFormData.groupIds.includes(g.id)));
          if (forLivestream) {
            setUpdateMode(true);
          }
        } else {
  ...
  }, [livestreamId, allFetched, draftStreamId, groupId, existingInterests, existingGroups]);
