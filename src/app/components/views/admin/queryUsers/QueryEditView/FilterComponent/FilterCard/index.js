...
const FilterCard = ({ filter, handleRemoveGroupFromFilters, groupsLoaded }) => {
   ...
   const group = useSelector(
      (state) => state.firestore.data.careerCenterData?.[groupId]
   );
   const totalFollowers = useSelector(
      (state) => state.firestore.ordered?.[`followers of ${groupId}`],
      shallowEqual
   );
   ...
   const filteredData = useSelector((state) =>
      Boolean(state.currentFilterGroup.data.filteredStudentsData.data)
   );
   ...
   const handleCheckForRedundantFilters = (
      group,
      groupFilterOptions,
      groupId
   ) => {
      ...
      if (check.hasRedundant) {
         handleRemoveRedundantFilterOptions(
            check.filteredGroupFilterOptionCategoryIds,
            groupId
         );
      }
   ...
   const hasRedundantFilterOptions = (groupFilterOptions, groupCategories) => {
      const filteredGroupFilterOptionCategoryIds = groupFilterOptions
         .map((category) => category.categoryId)
         .filter((categoryId) => categoryExists(groupCategories, categoryId));
      return {
         hasRedundant: checkIfRedundant(
   ...
   const handleRemoveFilterOption = useCallback(
      (targetCategoryId, groupId) => {
         const newFilterOptions = filterOptions
            .filter((option) => option.categoryId !== targetCategoryId)
   ...
   const handleRemoveRedundantTargetOptions = (
      targetOptionIds,
      categoryId,
      groupId
   ) => {
      dispatch(
         actions.setFilterOptionTargetOptions(
            targetOptionIds,
            categoryId,
            groupId
         )
   ...
   const handleCheckAndRemoveRedundantTargetOptions = (
      groupFilterOptions,
      group
   ) => {
      ...
      groupFilterOptions.forEach((filterOption) => {
         if (filterOption.targetOptionIds) {
            const filteredTargetOptions = filterOption.targetOptionIds.filter(
               (optionId) =>
                  flattenedGroupOptions.some(({ id }) => id === optionId)
            );
            const isRedundant = checkIfRedundant(
               filterOption.targetOptionIds,
               filteredTargetOptions
            );
            if (isRedundant) {
               handleRemoveRedundantTargetOptions(
                  filteredTargetOptions,
