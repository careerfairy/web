...
const DEFAULT_ERROR_MESSAGE =
   "An Error occured while joining the stream. Please contact support.";

const errorMessages = {
   UNEXPECTED_ERROR:
      "An unexpected error occured. Please contact support for more information.",
   UNEXPECTED_RESPONSE:
      "An unexpected error occured on the server. Please contact support for more information.",
   INVALID_PARAMS: "",
   NOT_SUPPORTED:
      "Your browser does not support certain connection operations. Please use Google Chrome v74+ for macOS/Windows and try again.",
   INVALID_OPERATION:
      "Some invalid operation was performed. Please contact support.",
   OPERATION_ABORTED:
      "The operation failed due to poor network conditions. Please check your connection.",
   WEB_SECURITY_RESTRICT:
      "Some browser security restrictions are not met. Please contact support.",
   NETWORK_TIMEOUT:
      "The operation failed due to poor network conditions. Please check your connection.",
   ...
   RTC_MEDIA_PERMISSION_DENIED:
      "Your browser is missing permissions to access your camera and/or microphone. Check these permissions and try again.",
   RTC_MEDIA_OPTION_INVALID: DEFAULT_ERROR_MESSAGE,
   RTC_DEVICES_NOT_FOUND:
      "Your browser could not detect any media devices. Are your camera and microphone connected?",
   RTC_NOT_SUPPORTED:
      "This browser is not supported by CareerFairy. Try using an up-to-date version of Google Chrome or Microsoft Edge",
   RTC_CONSTRAINT_NOT_SATISFIED:
      "This camera and/or microphone are not supported by CareerFairy",
   ...
   RTC_PUBLISH_STREAM_FAILED:
      "It seems like you could not connect to CareerFairy. Check your VPN and firewall settings.",
   RTC_PEERCONNECTION_FAILED:
      "It seems like you could not connect to CareerFairy. Check your VPN and firewall settings.",
   RTC_REQUEST_ABORT: DEFAULT_ERROR_MESSAGE,
   RTC_SCREEN_SHARE_NOT_ALLOWED:
      "Your browser does not have permission to share your screen. Please check your computer's privacy settings.",
};
   ...
   return (
      <>
         ...
         <Typography style={{ marginBottom: 10 }}>
            {reconnectTimeout
               ? "Check your connection. Attempting to reconnect..."
         ...
         <Button
            onClick={clearSoundWarning}
            size="small"
            disabled={!playWarning}
         >
            Mute Sound
         </Button>
      </>
...
function ErrorModal({
   ...
   const [hasHadInitialConnection, setHasHadInitialConnection] = useState(
      false
   );
   ...
   const notifyAudio = new Audio(
      "https://firebasestorage.googleapis.com/v0/b/careerfairy-e1fd9.appspot.com/o/static_files%2Falert_simple.wav?alt=media"
   );

   const errorInRtc = agoraRtcStatus && agoraRtcStatus.type === "ERROR";
   const errorInRtm = agoraRtmStatus && agoraRtmStatus.type === "ERROR";
   const errorInRtcConnection =
      hasHadInitialConnection &&
      (agoraRtcStatus.curState === "DISCONNECTED" ||
         agoraRtcStatus.curState === "CONNECTING");
   const showError =
         ...
         <DialogContent>
            <div className={classes.container}>
               <div className={classes.content}>
                  ...
                  {(errorInRtm || errorInRtcConnection) && (
                     <NetworkError
                        clearSoundWarning={clearSoundWarning}
