...
const useStyles = makeStyles((theme) => ({
   ...
   animatedBox: {
      transition: ({ durationTransform }) =>
         `transform ${durationTransform}ms ease-in, opacity ${durationTransform}ms cubic-bezier(1,0,.83,.67)`,
      position: "absolute",
...
const ActionButton = React.memo(
   ({
      ...
      const durationTransform = useMemo(() => getRandomDuration(3500, 4500), [
         id,
      ]);
      ...
      return (
         <div className={classes.animatedBox}>
            <RubberBand style={{ position: "absolute" }}>
               <div className={classes.actionBtn}>{renderIcon()}</div>
            </RubberBand>
         </div>
      );
   },
