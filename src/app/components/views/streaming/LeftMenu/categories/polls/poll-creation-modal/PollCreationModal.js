...
function PollCreationModal({
   ...
   function savePoll() {
      ...
      if (initialPoll) {
         firebase
            .updateLivestreamPoll(streamRef, initialPoll.id, question, options)
            .then(() => {
               resetForm();
            });
      } else {
         firebase
            .createLivestreamPoll(streamRef, question, options)
            .then(() => {
               resetForm();
            });
      }
      ...
      return (
         <Grow key={id} in>
            <TextField
               ...
               helperText={
                  text?.trim() === "" && error && "Please fill or remove"
               }
               variant="outlined"
               margin="dense"
               fullWidth
