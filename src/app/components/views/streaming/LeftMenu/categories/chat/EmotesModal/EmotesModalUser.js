...
const EmotesModalUser = ({displayName, email, initials, avatar, emojiSrc, emojiAlt, handleUnEmote, loading, prop}) => {
    const {userData} = useAuth()
    const {isStreamer} = useCurrentStream()
    const classes = useStyles()
    const [isMe, setIsMe] = React.useState(userData?.userEmail === email ||(isStreamer && email === "test@careerfairy.io"));

    useEffect(() => {
        setIsMe(userData?.userEmail === email ||(isStreamer && email === "test@careerfairy.io"))
    }, [userData, email])

    const handleClickUser = () => isMe && handleUnEmote(prop, email)

    return (
        <ListItem onClick={handleClickUser} disabled={loading} button={isMe} alignItems="flex-start">
            <ListItemAvatar>
