...
const AudienceCategoryChart = ({ className, audience, firebase, ...rest }) => {
   ...
   useEffect(() => {
      function removeCareerCentersWithNoCategories() {
         setCareerCenters(
            unfilteredCareerCenters.filter((cc) => cc.categories?.length)
         );
      }
   ...
   useEffect(() => {
      if (currentGroup.categories?.length) {
         if (localColors.length < typesOfOptions.length) {
            ...
            setLocalColors([
               ...colorsArray,
               ...typesOfOptions.map(() => getRandomColor()),
            ]);
         }
      }
   ...
   return (
      <Card className={clsx(classes.root, className)} {...rest}>
         <CardHeader className={classes.header} title="Student Distribution" />
         <Tabs
            value={value}
         ...
         <CardContent>
            ...
            >
               {hasNoData() ? (
                  ...
                  <Doughnut data={data} ref={chartRef} options={chartOptions} />
               )}
            </Box>
            {!hasNoData() && (
