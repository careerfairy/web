...
const WifiIndicator = ({isDownLink, uplink, agoraRtcStatus, agoraRtmStatus, downlink, className, ...rest}) => {

    const [defaultPosition, setDefaultPosition] = useState(undefined);
    const dispatch = useDispatch();

    useEffect(() => {
        if (agoraRtcStatus.curState === "CONNECTING") {
        ...
        if (agoraRtcStatus.prevState === "CONNECTED" && agoraRtcStatus.curState === "DISCONNECTED") {
    ...
    const rtcConnectionInfo = useMemo(() => getRtcConnectionInfo(agoraRtcStatus.curState), [agoraRtcStatus.curState])
