...
function CurrentSpeakerDisplayer(props) {
   ...
   function getVideoContainerHeight(streamId) {
      if (props.isPlayMode) {
         ...
         if (props.streams.length > 0) {
            if (streamId === props.currentSpeaker) {
               return windowSize.width > 768 || !props.isViewer
                  ? `calc(80vh - ${topDisplacement}px)`
                  : "80vh";
            } else {
               return "20vh";
   ...
   function getVideoContainerClass(streamId, prop) {
      ...
      if (props.isPlayMode) {
         if (props.streams.length > 1) {
            return streamId === props.currentSpeaker
               ? `${prop}SpeakerVideo`
               : `${prop}QuarterWidth`;
         } else {
   ...
   if (!props.isPlayMode) {
      ...
      const localSpeaker = props.currentLivestream.liveSpeakers?.find(
         (speaker) => speaker.speakerUuid === props.localId
      );
      let localVideoElement = (
         <div key={"localVideo"} className={classes[localVideoClass]}>
            <LocalVideoContainer
               ...
               small={localVideoClass.includes("QuarterWidth")}
               {...props}
            />
         </div>
      );
      ...
      <React.Fragment>
         ...
         <div className={classes.relativeContainer}>
            <div
               ...
               style={{ height: getMinimizedSpeakersGridHeight() }}
            >
