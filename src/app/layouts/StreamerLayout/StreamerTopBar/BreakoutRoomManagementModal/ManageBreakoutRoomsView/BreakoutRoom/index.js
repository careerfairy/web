         ...
         <Grid item xs={mobile ? 2 : 2}>
            {roomId !== breakoutRoomId ? (
               <React.Fragment>
                  ...
                  >
                     <MenuItem onClick={handleGoToRoomWithAutoProfile}>
                        Join as {joinAsMenuData?.profile?.firstName}
                     </MenuItem>
                     <MenuItem onClick={handleGoToRoomWithNewProfile}>
                        Join with a different profile
                     </MenuItem>
                  </Menu>
               </React.Fragment>
            ) : (
...
const BreakoutRoom = ({
   ...
   const links = useStreamToken({
      forStreamType: "breakoutRoom",
      targetBreakoutRoomId: id,
   });
   ...
   useEffect(() => {
      const newBreakoutRoomLink = isMainStreamer
   ...
   const closeDeleteBreakoutRoomModal = () =>
      setDeleteBreakoutRoomModalOpen(false);
   ...
   const handleCloseRoom = async (event) => {
      ...
      try {
         setLoading(true);
         await updateBreakoutRoom(
            { hasStarted: false, hasEnded: true },
            id,
            livestreamId
         );
      } catch (e) {
   ...
   const handleJoinRoom = async (event) => {
      ...
      const prevProfile = liveSpeakers?.find(
         (speaker) => speaker.speakerUuid.replace(id, "") === streamerId
      );
      if (prevProfile) {
         handleOpenJoinAsMenu(
            event.currentTarget,
            breakoutRoomLink,
            prevProfile
         );
   ...
   const handleGoToRoom = async (auto) => {
      ...
      handleClose();
      window.location.href = addQueryParam(
         breakoutRoomLink,
         auto ? "auto=true" : ""
      );
   };
                  ...
                  >
                     ...
                     <Grid item xs={mobile ? (hasStarted ? 2 : 3) : 2}>
                        <Chip
                           ...
                           variant={
                              breakoutRoomId === id ? "outlined" : "default"
                           }
