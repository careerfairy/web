...
const Content = ({breakoutRooms, fullyOpened, handleClose, handleBackToMainRoom, mobile}) => {
    ...
    const closeProps = () => ({className: classes.modalButton, onClick: handleClose})
    ...
    useEffect(() => {
        if (livestreamId) {
            const unsubscribe = listenToBreakoutRoomSettings(livestreamId, settingsSnap => {
                ...
                const newlyOpenedRooms = getNewlyOpenedRooms(prevOpenRooms.current, newOpenRooms)
            ...
            dispatch(actions.enqueueSnackbar({
                ...
                options: {
                    ...
                    action: <BreakoutSnackAction
                        handleCloseSnackbar={() => handleCloseAction(snackbarKey)}
                        handleClickConfirm={() => handleClickConfirmSnackbar(snackbarKey)}
                    />,
                    anchorOrigin: {
