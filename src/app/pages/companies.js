            ...
            <Grid item xs={12} sm={4}>
                <div className='companies-container-element'>
                    <div className='companies-container-image'>
                        <img id='companies-container-image-tag' src={company.logoUrl} style={{ margin: 0, position: 'absolute', top: '50%', maxHeight: '100%', maxWidth: '60%', height: 'auto', width: 'auto', transform: 'translateY(-50%)'}}/>
                    </div>
                    <Typography id='companies-company-name' variant='h3' style={{ marginBottom: 20 }}>{company.name}</Typography>
                    <p>{company.industry}</p>
                    <p className='hiring-label'>hiring from</p>
                    <ElementTagList fields={company.fieldsHiring}/>
                    <Button style={{ position: 'absolute', bottom: '10px', left: '0', right: '0', width: '94%', margin:'0 auto'}} variant='contained' onClick={() => goToCompany(company.id)} color='primary'>Discover {company.name}</Button>
                </div>
                <style jsx>{`
