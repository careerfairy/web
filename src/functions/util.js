...
const formatHour = (LuxonTime) => {
    return `${LuxonTime.hour}:${(LuxonTime.minute < 10 ? ('0' + LuxonTime.minute) : LuxonTime.minute)} ${LuxonTime.offsetNameShort}`;
}

const getLivestreamTimeInterval = (livestreamStartDateTime, livestreamTimeZone) => {
    var startDateTime = DateTime.fromJSDate(livestreamStartDateTime.toDate(), { zone: livestreamTimeZone }).toFormat('HH:mm ZZZZ', { locale: "en-GB" });
...
const generateEmailData = (livestreamId, livestream, startingNow) => {
    ...
    if (startingNow) {
        return {
            ...
            to: recipientEmails,
            subject: 'NOW: Live Stream with ' + livestream.company + ' ' + getLivestreamTimeInterval(livestream.start,  livestream.timezone || 'Europe/Zurich' ),
        ...
        return {
            from: "CareerFairy <noreply@careerfairy.io>",
            to: recipientEmails,
            subject: 'Reminder: Live Stream with ' + livestream.company + ' ' + getLivestreamTimeInterval(livestream.start, livestream.timezone || 'Europe/Zurich' ),
            template: 'registration-reminder',
            "recipient-variables": JSON.stringify(recipientVariablesObj),
