...
export const setStreamsFromTimeframeAndFuture =
   (streams) => async (dispatch) => {
      dispatch({
         type: actions.SET_STREAMS_FROM_TIMEFRAME_AND_FUTURE,
         payload: streams,
      })
   }
...
export const selectVisibleStreams =
   (arrayOfNewVisibleStreamIds) => async (dispatch, getState) => {
      ...
      dispatch({
         type: actions.SET_VISIBLE_STREAM_IDS,
         payload: {
            hiddenStreamIds: Object.keys(newHiddenStreamIds).length
               ? newHiddenStreamIds
               : null,
            visibleStreamIds: arrayOfNewVisibleStreamIds,
         },
      })
   }
