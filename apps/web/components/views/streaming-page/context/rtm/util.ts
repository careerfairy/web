 ...
 */
export const createClient = (appId: string, config?: RtmConfig) => {
   ...
   function createClosure() {
      if (!client) {
         client = AgoraRTM.createInstance(appId, config)
      }
      return client
   }
...
export const createLazyClient = () => {
    ...
    */
   function createClosure(appId: string, config?: RtmConfig) {
      if (!client) {
         client = AgoraRTM.createInstance(appId, config)
      }
      return client
   }
   return (appId: string, config?: RtmConfig) => createClosure(appId, config)
}
/**
 * Returns a hook to access an RTM channel instance, use this outside your React component.
