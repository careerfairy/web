import SvgIcon, { SvgIconProps } from "@mui/material/SvgIcon"

export const WarningIcon = (props: SvgIconProps) => {
   return (
      <SvgIcon viewBox="0 0 48 48" {...props}>
         <svg
            xmlns="http://www.w3.org/2000/svg"
            width="48"
            height="48"
            viewBox="0 0 48 48"
            fill="none"
         >
            <path
               d="M20.5796 7.72001L3.63955 36C3.29029 36.6049 3.10549 37.2906 3.10353 37.9891C3.10158 38.6875 3.28254 39.3743 3.62841 39.9811C3.97428 40.5879 4.473 41.0935 5.07497 41.4477C5.67693 41.8019 6.36115 41.9923 7.05955 42H40.9396C41.638 41.9923 42.3222 41.8019 42.9241 41.4477C43.5261 41.0935 44.0248 40.5879 44.3707 39.9811C44.7166 39.3743 44.8975 38.6875 44.8956 37.9891C44.8936 37.2906 44.7088 36.6049 44.3596 36L27.4196 7.72001C27.063 7.13223 26.561 6.64625 25.9619 6.30898C25.3629 5.97171 24.687 5.79453 23.9996 5.79453C23.3121 5.79453 22.6362 5.97171 22.0372 6.30898C21.4381 6.64625 20.9361 7.13223 20.5796 7.72001V7.72001Z"
               stroke="#ADADC1"
               stroke-width="4"
               stroke-linecap="round"
               stroke-linejoin="round"
            />
            <path
               d="M24 18V26"
               stroke="#ADADC1"
               stroke-width="4"
               stroke-linecap="round"
               stroke-linejoin="round"
            />
            <path
               d="M24 34H24.02"
               stroke="#ADADC1"
               stroke-width="4"
               stroke-linecap="round"
               stroke-linejoin="round"
            />
         </svg>
      </SvgIcon>
   )
}
