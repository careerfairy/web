...
export const MockedStaticSparkCard = ({
   spark,
   stats,
}: StaticSparkCardProps) => {
   return (
      <Card variant="outlined" sx={styles.card}>
         <CardHeader
            ...
            title={spark.creatorName}
            subheader={`From ${
               spark.universityName.length > 30
                  ? spark.universityName.substring(0, 25) + "..."
                  : spark.universityName
            }`}
         ...
         <Box sx={{ position: "relative" }}>
            <Box sx={styles.sparksTypeAndTitle}>
               <SparkCategoryChip
                  categoryId={spark.categoryId as SparkCategory["id"]}
               />
            ...
            <CardMedia
               component="img"
               sx={styles.cardMedia}
               image={getResizedUrl(spark.thumbnailUrl, "md")}
         ...
         <CardActions sx={styles.cardActions}>
            <StatContainer
               ...
               value={stats.plays}
            />
            <StatContainer icon={<LikeIcon />} value={stats.likes} />
            <StatContainer icon={<ShareIcon />} value={stats.shareCTA} />
            <StatContainer
               icon={<TotalPlaysIcon />}
               value={stats.numberOfCareerPageClicks}
            />
         </CardActions>
      </Card>
   )
