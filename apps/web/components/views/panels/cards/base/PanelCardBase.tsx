   ...
   return (
      ...
      >
         <Box
            ...
            sx={combineStyles(styles.panelBackground, backgroundSx, {
...
type PanelDateBadgeProps = {
   startDate: Date | null | undefined
   variant: "small" | "default"
}

export const PanelDateBadge = ({
   startDate,
   variant = "default",
}: PanelDateBadgeProps) => {
   if (!startDate) {
   ...
   return (
      <Stack sx={styles.avatarsContainer} direction="row" spacing={0.5}>
         {logoUrls.map((logoUrl, index) => (
            <PanelHostAvatar key={index} logoUrl={logoUrl} size={size} />
         ))}
      </Stack>
   )
}

type PanelRegistrationStatusProps = {
   isRegistered: boolean
   variant?: "small" | "default"
}

export const PanelRegistrationStatus = ({
   isRegistered,
   variant = "default",
}: PanelRegistrationStatusProps) => {
