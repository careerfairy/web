...
import { makeOfflineEventDetailsShareUrl } from "util/makeUrls"
...
const Content = ({
   ...
   const viewerLink = useMemo(
      () =>
         offlineEvent
            ? makeOfflineEventDetailsShareUrl(offlineEvent.id, {
                 utm_source: "client",
                 utm_campaign: "offline-events",
                 utm_content: (
            ...
            : "",
      [offlineEvent, companyName, companyCountryCode]
   )
   ...
   const handleLinkedIn = () => {
      ...
      const message =
         ...
         `Then mark the date: ${offlineEvent.startAt
            ?.toDate()
            ?.toLocaleDateString()}.\n\n` +
         `We are hosting an offline event and we will introduce you to the world of [offline event topic].\n\n` +
         `Join us to learn more about:\n` +
         `- [add reasons why students should join the offline event]\n\n` +
   ...
   const handleQrCode = async () => {
      ...
      await generateAndDownloadQr({
         ...
         fileName: `offline-event-qr-${offlineEvent.id}.png`,
      })
   }
      ...
      <ResponsiveDialogLayout.Content sx={styles.container}>
         <Box sx={styles.imageContainer}>
            <Image
               ...
               alt="Offline event promo dialog background"
               fill
               quality={100}
               style={styles.image}
               sizes={
            ...
            />
            <IconButton
               data-testid="promote-offline-event-dialog-close-button"
               onClick={onClose}
               sx={styles.closeIcon}
            >
               <X />
         ...
         <Stack sx={styles.content}>
            <Stack textAlign="center" mb={2}>
               ...
               <Typography variant="medium" color="neutral.700">
                  Use this link to share your offline event with your talent
                  community!
               </Typography>
            </Stack>
            <Stack spacing={2.4375}>
               <BrandedTextField
                  label={hasCopied ? "Link copied!" : "Offline event link"}
                  value={viewerLink}
                  InputProps={{
                     endAdornment: (
                        <Box
...
export const ShareOfflineEventDialog = ({
   open,
   offlineEvent,
   companyName,
   companyCountryCode,
   ...
   return (
      <ResponsiveDialogLayout
         open={open}
         handleClose={onClose}
         dataTestId="promote-offline-event-dialog"
         hideDragHandle
         dialogPaperStyles={{
            maxWidth: 517,
         }}
