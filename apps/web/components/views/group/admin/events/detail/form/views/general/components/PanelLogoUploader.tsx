...
const FIELD_NAME = "general.panelLogoUrl"
...
const EmptyBanner = () => {
   return (
      <Box sx={styles.emptyBannerWrapper}>
         ...
         <Box>
            <Typography sx={styles.label}>Upload event logo</Typography>
            <Typography sx={styles.recommendedSize}>
               Recommended size: 450x200
            </Typography>
         </Box>
         <FileUploaderButton buttonLabel="Upload logo" />
      </Box>
   )
}
   ...
   return (
      <>
         <Box sx={styles.changeBannerButtonWrapper}>
            <FileUploaderButton buttonLabel="Change logo" hideIcon={true} />
         </Box>
         <Box
            component="img"
            src={imageUrl}
...
export const PanelLogoUploader = () => {
   ...
   const { fileUploaderProps, dragActive } = useFileUploader({
      ...
      onCancel: async () => {
         handleTouched()
         if (!field.value) {
            await helpers.setError("Please provide a panel logo")
         }
      },
   })
